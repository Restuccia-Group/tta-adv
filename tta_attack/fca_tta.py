...
class FCA_TTA:
    ...
    def generate_attack(self, sur_model, x, y):
        ...
        adv = (torch.zeros_like(x.clone())- x + 127.5/255).requires_grad_(True)
        ...
        for t in range(num_iter):
            ...
            adv.data = (adv.data +x).clamp(0,1) -(x)
    ...
    def calculate_centriod(self):
        ...
        for i in self._cls:
            self.centroid[i,:] = torch.mean(self.feat_for_centroid[self.pred_cls == i], 0)
    ...
    def feat_centroid_loss(self):
        ...
        feat_adv = self._features_adv.squeeze()
        centroid_copy = torch.zeros_like(feat_adv)
        index_list = self.pred_cls
    ...
    def feature_collapse(self, weight=None):
        mal_num = math.ceil(self.cfg.DIA.MAL_PORTION * self.cfg.DATA.BATCH_SIZE)
        feat = self._features_adv.squeeze()
        ...
        #loss = nn.L1Loss()
        loss_func = nn.CosineSimilarity(dim=1, eps=1e-6)
        distance = loss_func(feat, mu)
        return distance
    
    def feature_dispersion(self, weight=None):
        mal_num = math.ceil(self.cfg.DIA.MAL_PORTION * self.cfg.DATA.BATCH_SIZE)
        feat_clean = self._features.squeeze()
        feat_adv = self._features_adv.squeeze()
        loss_func = nn.CosineSimilarity(dim=1, eps=1e-6)
        #loss_func = nn.MSELoss()
...
class Activation_Hook():
    def __call__(self, module, input, output):
        module.act = output.clone() #detach().clone().cpu()
