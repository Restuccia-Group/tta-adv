...
class MomentumBN(nn.Module):
    def __init__(self, bn_layer, momentum):
        ...
        self.register_buffer("target_mean", torch.zeros_like(bn_layer.running_mean))
        self.register_buffer("target_var", torch.ones_like(bn_layer.running_var))
...
class MedBN2d(MomentumBN):
    ...
    def forward(self, x):
        if self.training:
            ...
            if self.bn_layer.track_running_stats and self.bn_layer.running_mean is not None and self.bn_layer.running_var is not None:
                mean = (1 - self.momentum) * self.source_mean + self.momentum * b_mean
                var = (1 - self.momentum) * self.source_var + self.momentum * b_var
            else:
                mean, var = b_mean, b_var
